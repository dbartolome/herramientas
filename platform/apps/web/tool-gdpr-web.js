...
function getCheckResultLabel(result) {
  ...
  if (result === "partial") return "Cumplimiento parcial";
  return "No cumple";
}
...
function buildReportFromScan(scan) {
  ...
  if (Array.isArray(result.checks) && result.checks.length > 0) {
    return {
      ...
      disclaimer:
        result.disclaimer ||
        "Evaluación técnica automatizada. Es una pre-auditoría y no sustituye una revisión legal especializada.",
...
function renderChecks(checks) {
  elements.checksGrid.innerHTML = checks
    .map((check) => {
      ...
      return `
      <article class="rgpd-check ${result}" data-check-id="${check.id}">
        <div class="rgpd-check-head">
...
function renderEvidence(report) {
  ...
  const shots = evidence.screenshotUrls || {};
  const beforeUrl = shots.before ? `${API_BASE_URL}/scans/evidence/${encodeURIComponent(shots.before)}` : "";
  const afterUrl = shots.after ? `${API_BASE_URL}/scans/evidence/${encodeURIComponent(shots.after)}` : "";
  ...
  elements.evidencePanel.innerHTML = `
    <article class="rgpd-check info">
      ...
      <p><strong>Política privacidad:</strong> ${evidence.privacyPolicyUrl || "-"}</p>
      <p><strong>Política cookies:</strong> ${evidence.cookiesPolicyUrl || "-"}</p>
      <div class="rgpd-links-row">
        ${beforeUrl ? `<a class="open-link" href="${beforeUrl}" target="_blank" rel="noopener noreferrer">Ver captura antes</a>` : ""}
        ${afterUrl ? `<a class="open-link" href="${afterUrl}" target="_blank" rel="noopener noreferrer">Ver captura después</a>` : ""}
      </div>
...
async function createScan(url) {
  const response = await fetch(`${API_BASE_URL}/scans`, {
    ...
    body: JSON.stringify({ type: "gdpr_web", asset: url, params: {} })
  });
