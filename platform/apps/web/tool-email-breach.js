...
function buildReportFromScan(scan) {
  ...
  if (Array.isArray(result.checks) && result.checks.length > 0) {
    return {
      ...
      disclaimer:
        result.disclaimer ||
        "Evaluación automática de exposición en brechas conocidas. No sustituye una respuesta completa de gestión de identidades y credenciales.",
  ...
  const checks = findings.map((f) => ({
    id: f.id,
    title: f.title,
    description: `Control de exposición: ${f.title}`,
  ...
  const hasBreach = checks.some((c) => c.id === "hibp_breaches" && c.result === "fail");
  ...
  return {
    ...
    evidence: {
      ...
      breachDetected: hasBreach ? "Sí" : "No",
...
function renderHistory() {
  if (!state.history.length) {
    elements.historyList.innerHTML = '<p class="subtitle">Aún no hay consultas en esta sesión.</p>';
    return;
  }
...
function renderChecks(checks) {
  elements.checksGrid.innerHTML = checks
    .map((check) => {
      ...
      return `
      <article class="breach-check ${result}" data-check-id="${check.id}">
...
function renderEvidence(report) {
  ...
  elements.evidencePanel.innerHTML = `
    <article class="breach-check info">
      ...
      ${typeof evidence.sensitiveBreaches === "number" ? `<p><strong>Brechas sensibles:</strong> ${evidence.sensitiveBreaches}</p>` : ""}
      ${Array.isArray(evidence.dataClasses) && evidence.dataClasses.length ? `<p><strong>Data classes:</strong> ${evidence.dataClasses.join(", ")}</p>` : ""}
...
function renderFailedScan(scan) {
  ...
  elements.error.hidden = false;
  elements.error.textContent = scan.result?.summary || "La consulta terminó con error.";
}
...
async function createScan(asset) {
  ...
  if (!response.ok) {
    throw new Error(payload.message || "No se pudo crear la consulta");
  }
...
async function handleSubmit(event) {
  ...
  try {
    ...
    finishProgress();
    renderError(`Error en consulta: ${error.message}`);
  } finally {
