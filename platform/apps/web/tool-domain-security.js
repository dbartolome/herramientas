...
function computeScore(checks) {
  ...
  for (const check of checks) {
    const weight = weightBySeverity[check.severity] || 1;
    total += weight;
    if (check.result === "pass") earned += weight;
    if (check.result === "partial") earned += weight * 0.5;
  }
...
function buildReportFromScan(scan) {
  ...
  const checks = findings.map((finding, index) => ({
    id: finding.id || `check-${index + 1}`,
    title: finding.title || "Check DNS",
    description: `Control DNS: ${finding.title || "sin título"}`,
    severity: finding.severity || "info",
    result: mapSeverityToResult(finding.severity || "info"),
    evidence: finding.evidence || "Sin evidencia adicional",
    recommendation: finding.recommendation || "Revisar configuración DNS",
    details: finding
  }));

  const { score, status } = computeScore(checks);
  const topRisks = checks.filter((check) => check.result !== "pass").slice(0, 3);
  ...
  return {
    ...
    status,
    disclaimer:
      "Evaluación automática de postura DNS. No sustituye una auditoría completa de arquitectura DNS ni revisión operativa de zona.",
    checks,
    topRisks,
    evidence: {
      summary: result.summary || "Sin resumen adicional",
      ns: checks.find((c) => c.id === "dns_ns")?.evidence || "n/d",
      resolution: checks.find((c) => c.id === "dns_a")?.evidence || "n/d",
      caa: checks.find((c) => c.id === "dns_caa")?.evidence || "n/d",
      txt: checks.find((c) => c.id === "dns_txt")?.evidence || "n/d"
    }
  };
}
...
function renderChecks(checks) {
  elements.checksGrid.innerHTML = checks
    .map((check) => {
      ...
      return `
      <article class="domain-check ${result}" data-check-id="${check.id}">
...
function renderEvidence(report) {
  ...
  elements.evidencePanel.innerHTML = `
    <article class="domain-check info">
      ...
      <p><strong>TXT:</strong> ${evidence.txt || "n/d"}</p>
...
async function createScan(asset) {
